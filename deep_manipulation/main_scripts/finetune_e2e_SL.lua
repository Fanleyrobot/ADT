--[[ File
@description:
    This file is for end-to-end fine-tuning a combinec (p+c) network using weighted losses.
@version: V0.22
...
cmd:option('-extra_ratio', 0.75, 'the ratio of real images in a minibatch')
-- End Addition
...
local s, l, p, extra_i, extra_p
s, l, p = sim_ob:get_batch_e2e(valiset_size)
if sim_ob.extra_dataset then
  -- extra_i, extra_p = sim_ob:get_batch_extra(valiset_size*extra_ratio/(1-extra_ratio))
  extra_i, extra_p = sim_ob:get_batch_extra(valiset_size)
end
agent:sample_validation_data(s, l, p, extra_i, extra_p)
-- require 'image'
...
print("Validation set size: (s_q, s_i, v, p)", #s[1], #s[2], #l, #p)
print("Extra dataset: ", sim_ob.extra_dataset, type(extra_i), type(extra_p))
print("s_q (min max):", s[1]:min(), s[1]:max())
print("s_i (min max):", s[2]:min(), s[2]:max())
print("v (min max):", l:min(), l:max())
...
while step < opt.steps do
    ...
    if sim_ob.extra_dataset then
      -- extra_i, extra_p = sim_ob:get_batch_extra(minibatch_size*extra_ratio/(1-extra_ratio))
      extra_i, extra_p = sim_ob:get_batch_extra(minibatch_size)
    end
    agent:train(s, l, p, extra_i, extra_p)
    -- agent:train(s, l, p)
    -- print("Step: ", step)
    ...
    if step % opt.eval_freq == 0 then
        local cost, cost_sum, p_cost, p_cost_sum = agent:compute_validation_statistics()
        print("cost: ", cost)
        print("cost sum: ", cost_sum)
        print("perception cost: ", p_cost)
        print("perception cost sum: ", p_cost_sum)

        local cost_sum_history_tensor = torch.Tensor(agent.cost_sum_history)
        local p_cost_sum_history_tensor = torch.Tensor(agent.p_cost_sum_history)
        ...
        if pcall(plotTensorToFile, p_cost_sum_history_tensor, 'cost sum history', 'Training Epochs', 'cost sum', opt.plot_filename.."_p.png") then
            -- no errors
        else
            -- plotTensor raised an error
            print("plotTensor raised an error")
    ...
    if step % opt.save_freq == 0 or step == opt.steps then
        ...
        local p_model = agent.p_network:clone()
        ...
        torch.save(filename_ .. ".t7", {model = model,
                                ...
                                p_cost_history = agent.p_cost_history,
                                p_cost_sum_history = agent.p_cost_sum_history})
