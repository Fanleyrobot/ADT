...
if real_ob then
    ...
    local s0_r, s_r, l_r = real_ob:get_batch_ctrl(n_r)
    local s_s, l_s = sim_ob:get_batch_ctrl(valiset_size-n_r)
    s = torch.cat(s_r,s_s,1)
    l = torch.cat(l_r,l_s,1)
else
    s, l = sim_ob:get_batch_ctrl(valiset_size)
end
-- require 'image'
...
while step < opt.steps do
    ...
    if real_ob then
        ...
        local s0_r, s_r, l_r = real_ob:get_batch_ctrl(n_r)
        local s_s, l_s = sim_ob:get_batch_ctrl(minibatch_size-n_r)
        s = torch.cat(s_r,s_s,1)
        l = torch.cat(l_r,l_s,1)
    else
        s, l = sim_ob:get_batch_ctrl(minibatch_size)
    end
    agent:train(s, l)
